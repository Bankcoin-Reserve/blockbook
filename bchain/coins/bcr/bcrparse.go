...
func init() {
	// BCR mainnet Address encoding magics
	...
	// BCR testnet Address encoding magics
...
// BCRParser handle
type BCRParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewBCRParser returns new BCRParser instance
func NewBCRParser(params *chaincfg.Params, c *btc.Configuration) *BCRParser {
	p := &BCRParser{
...
// GetChainParams contains network parameters for the main BCR network
...
func (p *BCRParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
...
func (p *BCRParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *BCRParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *BCRParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *BCRParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *BCRParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *BCRParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *BCRParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
